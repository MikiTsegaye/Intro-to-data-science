...
def person_quotation_pattern_extraction(raw_text):
    pattern = r'([A-Z][a-z]+(?:\s[A-Z][a-z]+)?)\s*(?:said|answered|replied|responded)?\s*:?\s*"(.*?)"'
    ...
    if match:
        ...
        return person_name, quoted_text
    else:
        return None, None
...
def quotation_person_pattern_extraction(raw_text):
    ...
    text_pattern=r'"(.*?)"'
    name_pattern=r'(\w*\.*\s[A-Z][a-z]+\s[A-Z][a-z]+)(:| said| answered| responded| replied|)'
    extracted_quotation=re.findall(text_pattern,raw_text)[0]
    print(extracted_quotation)
    extracted_name=(re.findall(name_pattern,raw_text)[0][0]).strip()
    if(extracted_name,extracted_quotation)!=(None,None):
        return extracted_name,extracted_quotation
    else:
        return None,None
...
def transfer_raw_text_to_dataframe(filename):
    ...
    with open(filename, 'r') as file:
        for line in file.readlines():
            ...
            # Try first extraction function
            per_name, ext_text = person_quotation_pattern_extraction(line)
            
            # If the first fails, try the alternative pattern
            if per_name is None or ext_text is None:
                per_name, ext_text = quotation_person_pattern_extraction(line)
            
            # Only append if we found both
            if per_name is not None and ext_text is not None:
                person_name.append(per_name)
                extracted_text.append(ext_text)
...
def create_simple_pipeline():
    clf_pipeline=Pipeline([('vect',CountVectorizer()),('clf',MultinomialNB())])
    return clf_pipeline
...
def predict(clf_trained_pipeline, x_test):
    y_predict=clf_trained_pipeline.predict(x_test)
    return y_predict
